...
void LCD_write(uint8_t data, uint8_t mode){
  if(mode == LCD_COMMAND){
	GPIO_WriteBit(lcd_gpio.DCPORT, lcd_gpio.DCPIN, Bit_RESET);
	GPIO_WriteBit(lcd_gpio.CEPORT, lcd_gpio.CEPIN, Bit_RESET);
    LCD_send(data);
    GPIO_WriteBit(lcd_gpio.CEPORT, lcd_gpio.CEPIN, Bit_SET);
  }
  else{
	GPIO_WriteBit(lcd_gpio.DCPORT, lcd_gpio.DCPIN, Bit_SET);
	GPIO_WriteBit(lcd_gpio.CEPORT, lcd_gpio.CEPIN, Bit_RESET);
    LCD_send(data);
    GPIO_WriteBit(lcd_gpio.CEPORT, lcd_gpio.CEPIN, Bit_SET);
  }
 ...
 */
void LCD_init(){
  GPIO_WriteBit(lcd_gpio.RSTPORT, lcd_gpio.RSTPIN, Bit_RESET);
  GPIO_WriteBit(lcd_gpio.RSTPORT, lcd_gpio.RSTPIN, Bit_SET);
