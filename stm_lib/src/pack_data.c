...
void pack_world(struct data *data) {
  if (data-> number_of_world >= 1)
    memcpy(&data->world_1, &data->world_1 + 1, sizeof(uint8_t));
  if (data -> number_of_world >= 2)
    memcpy(&data->world_2, &data->world_2 + 1, sizeof(uint8_t));
  if (data -> number_of_world >= 3)
    memcpy(&data->world_3, &data->world_3 + 1, sizeof(uint8_t));
  if (data -> number_of_world >= 4)
    memcpy(&data->world_4, &data->world_4 + 1, sizeof(uint8_t));
  if (data -> number_of_world >= 5)
    memcpy(&data->world_5, &data->world_5 + 1, sizeof(uint8_t));
}
